...
app.put('/api/clients/:id', asyncRoute(async(req,res)=>{
  const allowed = ['name','phone','passport','source','assignedTo','status','notes'];
  const updates = {};
  for (const key of allowed) {
    if (Object.prototype.hasOwnProperty.call(req.body, key)) updates[key] = req.body[key];
  }
  const client = await Client.findByIdAndUpdate(req.params.id, {$set: updates}, {new:true, runValidators:true});
  if (!client) return res.status(404).json({error:'Client not found'});
  res.json(cleanId(client));
}));
